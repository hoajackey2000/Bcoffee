            ...
            <div class="col-lg-12">
                <div class="page-title-item text-center">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item active" aria-current="page">{{__('Order Details')}}</li> {{-- {{('Order Details')}} --}}
                        </ol>
                    </nav>
                </div>
            </div>
                                ...
                                <div class="order-details">
                                        ...
                                        <div class="order-info-area">
                                            <div class="row align-items-center">
                                                ...
                                                <div class="col-lg-4 print-btn">
                                                    <div class="prinit">
                                                        <a href="{{asset('assets/front/invoices/product/' . $data->invoice_number)}}" download="CHI_TIET_HOA_DON" id="print-click" class="btn"><i class="fas fa-print"></i>{{__('Download Invoice')}}</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="billing-add-area">
                                        <div class="row">
                                            ...
                                            {{-- dia chi giao hang --}}
                                            <div class="col-md-6">
                                                <div class="payment-information"> {{-- main-info --}}
                                            ...
                                            <div class="col-md-6 ">
                                                <div class="payment-information">
                                                    ...
                                                    <p>{{__('Payment Status')}} :
                                                        ...
                                                        <span class="badge badge-danger">{{__('UnPaid')}}  </span> {{-- {{$data->payment_status}} --}}
                                                        @else
                                                        <span class="badge badge-success">{{__('Paid')}}  </span> {{-- {{$data->payment_status}} --}}
                                                        @endif
                                                    </p>
                                                    @if (!empty($data->shipping_method))
                                                    <p>{{__('Shipping Method')}} : <span> {{$data->shipping_method}} </span></p>
                                                    @endif
                                                    <p>{{__('Shipping Charge')}} : <span class="amount">{{$data->currency_symbol_position == 'left' ? $data->currency_symbol : ''}} {{number_format($data->shipping_charge,0,',','.')}} {{$data->currency_symbol_position == 'right' ? $data->currency_symbol : ''}}</span></p>
                                                    <p>{{__('Paid amount')}} : <span class="amount">{{$data->currency_symbol_position == 'left' ? $data->currency_symbol : ''}} {{number_format($data->total,0,',','.')}} {{$data->currency_symbol_position == 'right' ? $data->currency_symbol : ''}}</span></p>
                                        ...
                                        <table class="table table-bordered">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{$data->currency_symbol_position == 'left' ? $data->currency_symbol : ''}}{{convertUtf8(number_format($order->price,0,',','.'))}}{{$data->currency_symbol_position == 'right' ? $data->currency_symbol : ''}}</td>
                                                    <td>{{$data->currency_symbol_position == 'left' ? $data->currency_symbol : ''}}{{convertUtf8(number_format($order->price * $order->qty,0,',','.'))}}{{$data->currency_symbol_position == 'right' ? $data->currency_symbol : ''}}</td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
